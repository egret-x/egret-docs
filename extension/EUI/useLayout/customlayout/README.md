...
namespace uilayout {
  ...
  export class RingLayout extends eui.LayoutBase {
    ...
    public updateDisplayList(unscaledWidth: number, unscaledHeight: number): void {
      ...
      if (this.target == null)
        return
      const centerX: number = unscaledWidth / 2// 获得容器中心的X坐标
      const centerY: number = unscaledHeight / 2// 获得容器中心的Y坐标
      const horizon: number = centerX / 2// 获得水平可用长度的一半
      const vertical: number = centerY / 2// 获得垂直可用长度的一半
      const radius = horizon > vertical ? vertical : horizon// 取小的为圆形半径
      ...
      for (let i: number = 0; i < count; i++) {
        ...
        const elementWidth: number = 0
        const elementHeight: number = 0
        const angle: number = 2 * Math.PI * i / count// 获得角度的大小
        const childX: number = centerX + radius * Math.sin(angle) - elementWidth / 2// 获得圆周点的X坐标
        const childY: number = centerY - radius * Math.cos(angle) - elementHeight / 2// 获得圆周点的Y坐标
